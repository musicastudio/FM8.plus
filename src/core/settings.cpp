...
void load(HMODULE self) {
    ...
    wchar_t buf[64];
    GetPrivateProfileStringW(s, L"gui_scale", L"1", buf, 64, p);        g_scale = (float)_wtof(buf);
...
void save() {
    ...
    wchar_t buf[64];
    swprintf(buf, 64, L"%.2f", g_scale);    WritePrivateProfileStringW(s, L"gui_scale", buf, p);
