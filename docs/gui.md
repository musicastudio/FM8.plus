...
`Core::serveForms(module)` registers three, all generated by `tools/gen_forms.py` from the local FM8:

| Resource | What FM8.plus changes |
|---|---|
| `FRM` 5 | FormMain header: the wordmark control's rect from (21,35,116,58) to (10,35,127,58) |
| `FRM` 15 | FormMainCompact: the same control |
| `PICTURE` 193 | the wordmark bitmap, 95x23 to 117x23, with the "+" drawn in the new space |

Every shim calls it before FM8 builds its GUI. From there the "+" is FM8's own artwork: FM8 draws it,
